...
struct Node
{
	...
	{
		return { std::max(lhv.val, rhv.val) };
	}
};
...
private:
	void _build(ll_t node, ll_t start, ll_t end)
	{
		...
		else
		{
			...
			m_tree[node] =
			{
				std::max(m_tree[2 * node].val, m_tree[2 * node + 1].val)
			};
		}
	}
	...
	ll_t _query(ll_t l, ll_t r, ll_t node, ll_t start, ll_t end) const
	{
		...
		auto mid = (start + end) / 2;
		return std::max(_query(l, r, 2 * node, start, mid), _query(l, r, 2 * node, mid + 1, end));
	}

public:
	RMQ(const std::vector<ll_t> &arr) :
		m_tree{ },
		m_ref_arr{ arr },
